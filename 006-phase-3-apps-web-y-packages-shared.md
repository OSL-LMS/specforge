...
**Nota sobre `check-boundaries.ts`.** Ancla cinco filas (1, 2, 3, 4, 15). Salieron **521 líneas** de `assert` sin framework, no las 150-250 que este PRD estimó — el grueso es la semántica de globs de CODEOWNERS, la resolución de imports entre árboles (incluidos los alias, que es por donde se colaría el fallo real) y las fixtures negativas que cada afirmación exige. Está en la línea de los checks que ya existen (`check-curriculum.ts` pasa de 500). El dato reemplaza a la estimación.
