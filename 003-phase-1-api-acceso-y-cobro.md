...
- **El nombre de cookie se verifica contra `apps/api`.** `api-client.ts` resuelve la cookie probando `__Secure-authjs.session-token` y luego `authjs.session-token`; si el nombre que encuentra no coincide con el `AUTH_COOKIE_NAME` configurado, falla ruidosamente al arrancar en vez de producir 401 para todo el mundo — el fallo que § 5.1 llama el más probable, y que sin esto es silencioso en esta dirección.
...
**Estructura del módulo.** `api-client.ts` se parte en dos piezas, y la razón es de testabilidad, no de estética: la mitad que decide (`mapResponse`: respuesta → `Access | { error: true }`) no puede tocar `next/headers`, para que sea importable con ruta relativa y extensión desde `scripts/` bajo Node pelado, que es el runner de las filas 34-39 (§ 9). La mitad que lee cookies y hace `fetch` vive aparte y solo corre bajo runtime de Next, y lo que depende de ella se verifica manualmente en § 10 paso 3.
...
- **El correo.** La regla es **de servicio, no de un `catch`**: `apps/api` monta un **filtro de excepciones global** que registra únicamente `err.name` y `(err as { cause?: { code?: string } }).cause?.code` — **nunca** `err.message`, `err.stack`, ni el objeto de error — y aplica a todas las rutas. `DrizzleQueryError` embebe los parámetros ligados dentro del mensaje (`drizzle-orm@0.45.2/errors.js`, `super(\`Failed query: ${query}\nparams: ${params}\`)`), y `params` incluye el correo; el `console.error` de hoy (`src/app/api/paddle/webhook/route.ts:84`) sí lo filtra a los logs de Railway, así que la propiedad que la versión anterior de este PRD decía "conservar" nunca existió.
  ...
  **Tiene que ser global y no del webhook**: `getAccess` y `ensureTrial` ejecutan las mismas consultas parametrizadas con el correo (`access.ts:47-52`, `:97-101`) y **no capturan** — el error se propaga a quien atienda la excepción, que por defecto no es código que el implementador escriba y por tanto no es código que piense en redactar. Filas 31 y 40 de § 9 cubren los dos caminos.
...
| 34 | Timeout → `{ error: true }` | Regresión | `mapResponse` ante una promesa abortada por `AbortSignal.timeout`. **Es la fila que demuestra que la política de § 5.3 puede existir**: sin timeout, el render se cuelga y nunca llega a la rama de error. | `../platform/scripts/check-access-bridge.ts` |
| 35 | Cookie de sesión troceada falla ruidosamente | Error | Dado un mapa de cookies con `<cookie>.0`, el resolutor lanza en vez de devolver un token truncado, que daría un 401 indistinguible de un fallo de secreto. | `../platform/scripts/check-access-bridge.ts` |
...
| 37 | Nombre de cookie que no coincide con `AUTH_COOKIE_NAME` → falla al arrancar | Regresión | Cierra la dirección silenciosa del desajuste de salt (§ 5.3). | `../platform/scripts/check-access-bridge.ts` |
| 38 | Sin `API_BASE_URL`, `apps/web` no arranca | Regresión | Goal 5 del lado Next, hermana de la fila 3. Sin ella, una variable ausente tumba el producto de pago de forma indistinguible de una caída de `apps/api`. | `../platform/scripts/check-access-bridge.ts` |
| 39 | Sin `ANALYTICS_SALT` no se emite | Regresión | El decisor del píxel devuelve "no emitir" en vez de calcular con sal vacía. Cierra el retroceso de privacidad descrito en § 8. | `../platform/scripts/check-access-bridge.ts` |
| 40 | Un fallo de base en `GET /v1/access` no filtra el correo al log | Regresión | El camino de acceso no captura (`access.ts:47-52`), así que lo atiende el filtro global. Hermana de la fila 31, camino distinto. | `../platform/apps/api/test/access.e2e-spec.ts` |
...
3. **Encender el flag.** `ACCESS_VIA_API=true` en el servicio Next. Aquí se prueba el puente de verdad.
   ...
   **Señales, nombradas** — "vigilar" sin métrica no es accionable por nadie que no sea quien lo escribió:
   ...
   - Latencia añadida por el salto: ADR-001 § 6 fija **+200 ms p95 sobre el primer token de `/api/chat`** como señal para devolver el endpoint a `apps/web`. Es el umbral contra el que se compara.

   **Verificaciones manuales** (§ 9 explica por qué no son filas automatizables): con el flag encendido, comprobar en el navegador que (a) `/chat` renderiza el chat con el historial y el selector de lección cargados, (b) apagando `apps/api`, `/chat` sigue renderizando y el primer mensaje devuelve 503 sin llamada a Anthropic, (c) ese 503 no emite `tutor_message_sent`, y (d) un 401 de `apps/api` no manda a `/signin`.
