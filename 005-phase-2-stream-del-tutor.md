...
**Respuesta 200**: `Content-Type: text/plain; charset=utf-8`, `Cache-Control: no-store`, cuerpo en streaming. Solo se reenvían los `content_block_delta` de tipo `text_delta`, como hoy (`route.ts:150-156`): los deltas de `thinking` no viajan.
...
Lo que el timeout protege es el caso que `api-client.ts` describe en su cabecera —un `apps/api` que acepta la conexión y no responde— y ese caso ocurre antes de la primera cabecera o no ocurre. `TUTOR_TIMEOUT_MS` es nueva, por defecto 10 000.
...
- **El llamante se va**: `req.on("close")` aborta el stream de Anthropic (`MessageStream.abort()`, presente en `@anthropic-ai/sdk@0.104.1`) y **no persiste**. En Express `close` **también dispara al terminar con normalidad**, así que el handler lleva una bandera de "ya cerré": sin ella se llamaría `abort()` sobre un turno completado, después de haber persistido.
...
| 22 | Ventana de 30 | Unit | 40 mensajes guardados + el nuevo → viajan 30 y el primero es `user`. | `../platform/apps/api/src/tutor/tutor.service.spec.ts` |
...
**Paso A — medir la línea base.** Añadir a `src/app/api/chat/route.ts` la misma línea `first_token_ms=` que llevará `apps/api`, con el `t=0` en la entrada del handler y **excluyendo el tramo del puente** (`route.ts:53-55`), que en el camino nuevo no existe. Desplegar y dejarla correr hasta tener turnos de varios días. Sin esto la señal de ADR-001 § 6 no se puede disparar: hoy nadie mide el tiempo hasta el primer token. *Rollback*: quitar la línea.
...
**Comprobación manual obligatoria de este paso**: en devtools, iniciar un turno y, **después del primer chunk**, forzar offline o abortar la conexión. El texto ya pintado tiene que **permanecer** en la burbuja, con el aviso de reintento añadido debajo; si la burbuja se vacía, la separación de `try/catch` no se hizo. Hace falta porque la fila 38 prueba la decisión —dada la fase del fallo, qué conservar— y no la clasificación: una implementación que conserve el `catch` único y pase siempre la fase "antes del stream" hace pasar el test con el fallo intacto.
...
Sin migraciones de base, sin servicios nuevos en Railway, sin cambios en `drizzle.config.ts`, en `src/auth.ts`, en `src/auth.config.ts`, en `src/middleware.ts` ni en los nueve `scripts/check-*.ts` existentes.
