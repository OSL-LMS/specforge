...
**Errors**: el cuerpo de error es un **objeto**, no una cadena — `AllExceptionsFilter:84-87` devuelve `exception.getResponse()` tal cual, y `tutor.service.ts:91-94` ya documenta por qué la forma de objeto es la correcta aquí.

**El panel NO ve estos códigos**, y decirlo evita una contradicción con § 5.3: el puente aplica la regla positiva —sólo un 200 con la forma esperada produce resultado, todo lo demás es `{error:true}`— así que el código se descarta en la frontera y el estudiante ve una sola línea de fallo. El cuerpo estructurado existe para el operador y para los logs, no para la interfaz. 404 y 409 no son alcanzables desde la interfaz: el panel sólo se pinta cuando la lección declara `evidenceKind`, y el slug sale de la misma lectura del currículo que produjo la opción. **El 400 sí lo era** —pegar `http://` o un puerto distinto de 443— y producía un mensaje de reintento falso, porque reintentar igual falla igual. Lo cierra `checkEvidenceUrl()` en `evidence-panel.ts` (fila 64b de § 9): una comprobación de forma en el navegador, **antes** de salir, que atrapa esquema y puerto y devuelve una línea que dice qué corregir. No replica el DTO —`net.isIP` no existe en el navegador y la forma del host es infrecuente— ni es un control de seguridad: el servidor sigue siendo la autoridad, y esto es un atajo de mensaje.
...
- **Sin `skipIf`**, el guard evalúa **todos** los throttlers registrados en **cada** petición, así que el tope de 60/min alcanza a toda ruta del servicio. El mecanismo exacto importa para poder probarlo: el `generateKey` por defecto es `sha256(clase-handler-throttler-tracker)`, así que `getTracker: () => "global"` quita el eje de credencial pero **no el de ruta** — sin `skipIf` cada handler recibe *su propio* cubo de 60/min, no uno compartido. Consecuencia: sólo se rompe visiblemente donde el endpoint está provisionado **por encima** de 60, y ése es el webhook de Paddle, al que `WEBHOOK_THROTTLE` da 600/min a propósito porque Paddle entrega en ráfaga. El turno del tutor, con 10/min, cabe dentro de 60 y no delata nada.
...
4. **Un presupuesto único de reloj, que además acota cada operación.** `EVIDENCE_TIMEOUT_MS` se convierte en un instante límite fijado **una vez antes del bucle**. No es un timeout por operación: con 3 saltos habría hasta 4 resoluciones y 4 peticiones, y un tope por operación no acota la suma. Pero **comprobar antes de empezar no acota lo que empieza**: una resolución lanzada a un milisegundo del límite corre después su duración entera. Así que cada operación recibe como tope **el tiempo restante**: `AbortSignal.timeout(restante)` para el `fetch`, y para el DNS **una instancia de resolutor construida con ese tope** —`new Resolver({ timeout: restante, tries: 1 })`—, con `resolver.cancel()` al vencer. El tope de c-ares es opción del constructor, no un método (`Resolver` no expone `setTimeout()`), así que el resolutor se crea por operación a través de una fábrica. `tries: 1` no es opcional: c-ares reintenta cuatro veces por defecto y `timeout` acota **cada intento**, no la consulta — con el defecto, el presupuesto se multiplicaría por cuatro. Sin esto el rebase se come el margen de `EVIDENCE_BRIDGE_TIMEOUT_MS`, que es el fallo que ese margen existe para evitar.
...
| 38 | **Servicio: sin URL en analítica** | unit | Guarda **estructural**, no espía: se leen todos los ficheros no-spec de `src/evidence/` y se afirma que ninguno menciona `AnalyticsService` ni `.track(`, con una aserción positiva delante para que no pase con una lista vacía. Un espía sería vacuo — `AnalyticsService` no está cableado en `EvidenceModule`, y no puede estarlo sin ensanchar `TutorEvent` en `shared`, que no está en la tabla de Impacted Projects de este PRD. Misma forma que `scripts/check-boundaries.ts`. Goal 8. | `../platform/apps/api/src/evidence/evidence.service.spec.ts` |
...
| 46 | **e2e: el tope global no alcanza a otras rutas** | e2e | Con el throttler global registrado, **el webhook de Paddle sigue admitiendo sus 600/min**: 65 peticiones seguidas no dan 429. Tiene que golpear el webhook y no el tutor — el cubo sin `skipIf` es **por handler** (§ 5.4), así que los 10/min del tutor caben dentro de 60 y esa mitad pasa igual con `skipIf` y sin él. Sólo un endpoint provisionado por encima de 60 delata la ausencia. | `../platform/apps/api/test/evidence.e2e-spec.ts` |
...
| 59 | **Golden: `toLessonOptions` no se ensanchó** | unit | Una aserción **sobre la función real**: `Object.keys(…).sort() === ["slug","title"]` para cada opción que devuelve `toLessonOptions`. Es lo que protege `/registro`, que es pública y sin login. La aserción que ya existía en `:93` se conserva, pero **no** cubre esto: corre sobre opciones que el script reconstruye a mano, así que seguiría verde con la función ensanchada. | `../platform/scripts/check-curriculum-golden.ts` |
...
| 64b | **Panel: la forma de la URL se comprueba antes de salir del navegador** | unit | `checkEvidenceUrl()` acepta `https://…` y `:443`, y rechaza `http://`, otro esquema, un puerto distinto y algo que no parsea. Su mensaje sale **por la misma decisión** que las demás líneas —neutro, `role="status"`, sin la palabra "error"— y **gana a `submitFailed`**: sin viaje no hay nada que reintentar, y "reinténtalo" sería el mensaje falso que esta fila cierra (§ 5.1). | `../platform/apps/web/scripts/check-evidence-bridge.ts` |
